...
const DETECTOR_MAP: Record<string, (input: HTMLVideoElement, confidence: number) => Promise<unknown>> = {
  ...
  object_detector: async (input, confidence) => detectObjects(input, confidence),
...
export function useVideoDetection() {
  ...
  const processVideo = useCallback(
    async (
      ...
      const totalFrames = Math.floor(videoEl.duration * videoEl.playbackRate * 30);
      ...
      try {
        ...
        while (frameIndex < totalFrames) {
          ...
          for (const item of settled) {
            ...
            switch (item.type) {
              case "face_detector":
                combined.faceDetections = item.result as DetectionResults["faceDetections"];
                break;
              case "face_landmarker":
                combined.faceLandmarks = item.result as DetectionResults["faceLandmarks"];
                break;
              case "hand_landmarker":
                combined.handLandmarks = item.result as DetectionResults["handLandmarks"];
                break;
              case "pose_landmarker":
                combined.poseLandmarks = item.result as DetectionResults["poseLandmarks"];
                break;
              case "object_detector":
                combined.objectDetections = item.result as DetectionResults["objectDetections"];
                break;
              case "image_segmenter":
                combined.segmentation = item.result as DetectionResults["segmentation"];
                break;
  ...
  return { frameResults, loading, progress, error, processVideo, cancel, reset };
}
