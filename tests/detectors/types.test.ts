...
describe("DETECTOR_LABELS", () => {
  ...
  it("has labels for all 7 detector types", () => {
...
describe("DETECTOR_COLORS", () => {
  it("has colors for all 7 detector types", () => {
...
describe("DetectionResults type", () => {
  ...
  it("allows partial detection results", () => {
    const result: DetectionResults = {
      processingTimeMs: 42,
      faceDetections: [{ boundingBox: { originX: 10, originY: 20, width: 100, height: 200 }, keypoints: [], score: 0.95 }],
    };
  ...
  it("allows all detection types simultaneously", () => {
    const result: DetectionResults = {
      ...
      handLandmarks: { handLandmarks: [], handedness: [], handednessScores: [] },
      poseLandmarks: { poseLandmarks: [] },
      objectDetections: [],
      segmentation: { confidenceMasks: [] },
      gestures: [],
    };
